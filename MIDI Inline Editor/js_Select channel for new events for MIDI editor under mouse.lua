--[[
ReaScript name: js_Select MIDI channel for new events for MIDI editor under mouse.lua
Version: 0.95
Author: juliansader
...
--[[
  ...
  * v0.95 (2019-05-05)
    + Works properly when filter is set to Solo or Invert.
]]
...
filterActive = tonumber(filterActive)

if not (channel and channel >= 1 and channel <= 16 and filter and filterActive) then
    reaper.MB("Could not find the active channel and/or filter settings in the take chunk./n/nPerhaps the take under the mouse has not yet been opened in an inline MIDI editor", "ERROR", 0)
    return
end
...
if filterActive&1 == 1 then menuStr = menuStr:gsub("Event", "!Event") end
...
if userChoice == 2 then -- toggle event filter
    newChannel = channel
    if filterActive&1 == 0 then
        newFilterActive = filterActive|1
...
chunk = chunk:gsub("(\nEVTFILTER )[%-%d]+( [%-%.%d]+ [%-%.%d]+ [%-%.%d]+ [%-%.%d]+ )[%-%d]+ [%-%d]+ ", 
                   ...
                   string.format("%i", newFilterActive) .. " ") -- EVTFILTER numbers channels 0-15
