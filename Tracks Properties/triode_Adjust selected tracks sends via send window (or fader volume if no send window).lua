...
-- @metapackage
-- @provides
--   [main] triode_Adjust selected tracks sends via send window (or fader volume if no send window)/Add 1db to selected tracks sends via send window (or fader volume if no send window).lua
--   [main] triode_Adjust selected tracks sends via send window (or fader volume if no send window)/Subtract 1db from selected tracks sends via send window (or fader volume if no send window).lua
