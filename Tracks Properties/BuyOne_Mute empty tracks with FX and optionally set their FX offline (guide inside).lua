...
suitable for muting or unmuting with this script and as long as such track
status hasn't changed to being armed, folder parent or to having receives.
In AUTO_RUN mode the mute can be prestistent if MUTE_PERSIST setting
...
becomes suitable for muting or unmuting with this script in AUTO_RUN mode.
...
the button is lit.
Since in AUTO_RUN mode the script consumes some CPU, especially with all
settings enabled, it'd be prudent to ascertain that using it is advantageous
over simply leaving tracks unmuted and their FX online or running the script
manually.

4. Enable MUTE_PERSIST setting to prevent unmuting tracks with no items
...
local t = {}
	...
	local has_receives = r.GetTrackNumSends(tr, -1) > 0 -- -1 stands for receives
		if no_items_and_fx and not armed and not folder and not has_receives then
...
function toggle_track_fx_off_online(tr, ON) -- ON is a provisional arg
local set = not ON -- set to OFF else to ON
	for i = 0, r.TrackFX_GetCount(tr)-1 do
	r.TrackFX_SetOffline(tr, i, set)
	end
	for i = 0, r.TrackFX_GetRecCount(tr)-1 do
	r.TrackFX_SetOffline(tr, 0x1000000+i, set)
	end
end


function conditions_are_met(t) -- for updating tracks
	...
	local tr = r.GetTrack(0,i)
		for k, v in ipairs(t) do
			...
			local folder = r.GetMediaTrackInfo_Value(tr, 'I_FOLDERDEPTH') == 1 and i ~= r.CountTracks(0)-1 -- folder parent and not last track
			local has_receives = r.GetTrackNumSends(tr, -1) > 0 -- got receives // -1 stands for receives
				...
				if UNMUTE and (armed or folder or has_receives) then return true end
...
function MUTE()
	...
	if AUTO_RUN then
		...
		elseif #store_empty_tracks() ~= #stored -- count of empty tracks with fx changes // fx are removed or items added
		then update = true
		elseif (UNMUTE or SET_FX_OFFLINE or MUTE_PERSIST) and conditions_are_met(stored) then -- empty track becomes armed, folder parent or gets receives or new fx are added
		update = true)
		end
	end
	...
	if update then
		...
		local armed = r.GetMediaTrackInfo_Value(tr, 'I_RECARM') == 1
		local folder = r.GetMediaTrackInfo_Value(tr, 'I_FOLDERDEPTH') == 1 and i ~= r.CountTracks(0)-1 -- not last track // OR has_child_tracks(tr) which is more expensive
		local has_receives = r.GetTrackNumSends(tr, -1) > 0 -- -1 stands for receives
			 if no_items and fx and not armed and not folder and not has_receives
			 then
			 r.SetMediaTrackInfo_Value(tr, 'B_MUTE', 1)
			 local set_offline = SET_FX_OFFLINE and toggle_track_fx_off_online(tr)
			 end
			 if UNMUTE then
				for k, v in ipairs(stored) do
					if tostring(tr) == v and (not no_items or not fx or armed or folder or has_receives) then
					r.SetMediaTrackInfo_Value(tr, 'B_MUTE', 0)
					local set_online = SET_FX_OFFLINE and toggle_track_fx_off_online(tr, 1) -- 1 is ON
					end
				end
			end
		end
...
local undo = not AUTO_RUN and r.Undo_BeginBlock()

MUTE()

local undo = not AUTO_RUN and r.Undo_EndBlock(string.format('Mute%s tracks with no items %s', (UNMUTE and '/unmute' or ''), (SET_FX_OFFLINE and string.format('and set their FX offline%s', (UNMUTE and '/online' or '')) or '')),-1)
