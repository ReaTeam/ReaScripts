-- @description Move edit cursor to previous dubious or missing items crossfade on selected tracks
-- @author amagalma
-- @version 2.00
-- @changelog Complete re-write for better speed and fixed item lanes support
-- @link https://forum.cockos.com/showthread.php?t=241010
-- @donation https://www.paypal.com/paypalme/amagalma
-- @about
--   - Moves the edit cursor to the previous dubious or missing crossfade between two items on the selected tracks.
...
local function GetPreviousDubiousIntersection( track, wanted_lanes )
  ...
  for it = item_cnt-1, 0, -1 do
    local ptr = reaper.GetTrackMediaItem( track, it )
    local start = reaper.GetMediaItemInfo_Value( ptr, "D_POSITION" )
    local ending = start + reaper.GetMediaItemInfo_Value( ptr, "D_LENGTH" )
    local item = {
      ptr = ptr,
    ...
    if item.ending < cur_pos and previous_item_in_lane[item.lane] then
      if IsDubiousIntersection( item, previous_item_in_lane[item.lane] ) then
        break
      end
    end
...
for tr = 0, track_cnt-1 do
  ...
  GetPreviousDubiousIntersection( track, wanted_lanes )
end
...
if bad_fade[bf_cnt] then
  reaper.PreventUIRefresh( 1 )
  reaper.SetEditCurPos( bad_fade[bf_cnt].time, true, false )
  reaper.SelectAllMediaItems( 0, false )
  for item in pairs(bad_fade[bf_cnt].items) do
    reaper.SetMediaItemSelected( item, true )
  end
  reaper.PreventUIRefresh( -1 )
  reaper.UpdateArrange()
  reaper.Undo_OnStateChange( "Move to previous dubious item intersection" )
else  
  return reaper.defer(function() end)
end
