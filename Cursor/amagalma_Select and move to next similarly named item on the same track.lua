-- @description Select and move to next similarly named item on the same track
-- @author amagalma
-- @version 1.00
...
local next_item_ip = reaper.GetMediaItemInfo_Value( item, "IP_ITEMNUMBER" ) + 1
local last_item_ip = reaper.CountTrackMediaItems( track ) - 1
if next_item_ip > last_item_ip then return end
...
for i = next_item_ip, last_item_ip do
  ...
  if levenshtein_similarity(wanted_name, name) >= 0.5 then
    ...
    reaper.Undo_EndBlock2( 0, "Select next similarly named item", 4 )
    break
  end
end
