...
--   - Complete engine rewrite: ultra-optimized single defer loop with O(1) marker-track caching.
--   - Introduced predictive lookahead for cross-track recording transitions.
--   - Real-time MIDI data throttling to prevent performance stutter, avoiding O(N) evaluations in large projects.
--   - Real-time deterministic timeout tracking for ghost-take audio cleanup.
--   - "Auto-kill" logic terminates script cleanly when playback halts.
--   - Automatic Loop Splitting: Slices dragged-out loop items cleanly into loops at script initialization.
--   - Miniscule (5ms) linear crossfades applied to playback audio automatically to prevent waveform clicks.
--   - Added 'monitor' naming keyword to dedicate tracks to input-pass-through rather than recording.
--   - Added 'manual' naming keyword to allow per-track suppression of auto-fades.
--   - Added alias support explicitly allowing markers to fire track switches for tracks prefixed with 'manual'.
-- @link Looper for Reaper - ILOVEMEDIA https://ilovemedia.es/looper-reaper/

-- @about
--   # ReaLoop: Arranged Live Looper v2.0
--
--   ## Main Features
--   * Hands-free live looping for audio and MIDI tracking.
--   * Instantaneous auto-punch and automatic loop tracking.
--   * Dynamically switch tracks implicitly using Playback Markers.
--   * Repeat recording sections anywhere on the timeline via loop items.
--
--   ## Prepare your template for live looping
--   * Create empty `MIDI` placeholder items (Insert -> New MIDI Item) for both recording slots and playback slots.
--   * Extend the playback slots as much as you need; ReaLoop will cut and loop them perfectly.
--   * Format the `Take Name` attribute of these placeholder items:
--     - Add `live` (or your configured `recordName`) to the end to designate the slot as a LIVE recording block.
--     - Add `loop` (or your configured `playbackName`) to the end to designate the slot as a PLAYBACK block.
--   * Place project markers directly over the timeline where you want ReaLoop to arm the next instrument track. Name the marker identically to the track name.
--
--   ## Track Modifiers
--   * **Manual Fades:** Prepend `manual` to your track name (e.g. "manual drums"). ReaLoop will NOT apply automatic 5ms click-fades to audio loops on this track, leaving fades to your discretion. Note: Markers named "drums" will still perfectly trigger "manual drums".
--   * **Monitor Only:** Prepend `monitor` to your track name (e.g. "monitor vocals"). The script will disarm this track from recording so you can jam freely while looping continues playback.
--
