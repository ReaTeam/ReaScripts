...
-- @version 1.20
-- @provides [windows] .
-- @screenshot In Action https://stash.reaper.fm/34070/Reset%20VU%20Meters.gif
  ...
  for i = 1, #adrs do
    ...
    for j = 1, #childs do
      ...
      if reaper.JS_Window_GetClassName(childs[j], "", 13) == "REAPERvertvu" then
        reaper.JS_WindowMessage_Post(childs[j], "WM_LBUTTONDOWN", 0x0001, 1, 0, 0)
        reaper.JS_WindowMessage_Post(childs[j], "WM_LBUTTONUP", 0x0000, 0, 0, 0)
      end
    end
  end
end
