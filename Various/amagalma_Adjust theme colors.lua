...
function TableContentsAreEqual(t1, t2)
  for k,v in pairs(t1) do
    --reaper.ShowConsoleMsg(k .. "  " .. tostring(t1[k]) .. "  " .. tostring(t2[k]) .."\n")
...
function AB()
  ...
  if current == 1 and cur_version == 1 then
    GUI.settooltip( "Already viewing original theme colors" )
    return
  end
...
function TakeSnapshot()
  ...
  SetSliders(0, 0, 1)
  -- Update listbox
  GUI.elms.Compare.list[#GUI.elms.Compare.list+1] = "Adjusted colors Snapshot #" .. snapshot
  ...
  if not self:overscrollbar() then
    ...
    if GUI.mouse.cap & 16 == 16 then
      if item == 1 then -- protect original colors setting
        GUI.settooltip( "Can't delete original theme colors" )
        return
      else
        if AB_mode then
...
function GUI.Tabs:onmousedown()
  ...
  if previous_state == 2 and mouseopt == 1 then
    ...
    if not TableContentsAreEqual(GUI.elms.Groups.optsel, group_settings) then
      group_settings = {table.unpack(GUI.elms.Groups.optsel)}
      MakeColorGroupTable(GUI.elms.Groups.optsel)
      TakeSnapshot()
      GUI.settooltip("A Snapshot was automatically taken because\nthe Color Group selection has been changed")
    end
  end
  self.state = mouseopt
  self:redraw()
...
function exit()
  ...
  if TableContentsAreEqual(col[current][cur_version], col[1][1]) then
