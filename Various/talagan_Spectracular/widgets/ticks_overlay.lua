...
function TicksOverlay:drawTick(spectrograph, draw_list, time, color, thickness)
    ...
    if xp < self.x or xp > self.x + self.w then return end

    ImGui.DrawList_AddLine(draw_list,
        xp, self.y,
        xp, self.y + self.h,
        color, thickness or 1.0)
end
...
function TicksOverlay:drawGrid(ctx)
    ...
    local meas_num, meas_qns, _ = reaper.TimeMap_QNToMeasures(0, qns)
    ...
    while meas_num and tick_time <= view_bounds.time_stop do
        ticks[#ticks+1]         = { time=tick_time, is_measure=(tick_qn == meas_qns), measure_num = meas_num }
...
function TicksOverlay:drawCursor(ctx, draw_list)
    ...
    if self:containsPoint(mx, my) and not spectrograph.lr_mix_widget:containsPoint(mx, my) and not self.mw.rmse_widget.dragged and not self.mw.rmse_widget.is_hovering_a_label then
        ...
        ImGui.DrawList_AddRectFilled(draw_list, spectrograph:timeToX(l), self.y, spectrograph:timeToX(r), self.y + self.h, T.FFT_WINDOW_BG)
