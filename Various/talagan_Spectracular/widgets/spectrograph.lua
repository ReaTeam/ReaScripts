...
function SpectrographWidget:recalculateBitmap(ctx)
    ...
    self.rgb_buf = DSP.ensure_array_size(self.rgb_buf, sac.slice_count * sac.slice_size)
    ...
    DSP.analysis_data_to_rgb_array(sac.spectrograms, coeffs, self.rgb_buf, self.dbmin, self.dbmax)

    local t1 = reaper.time_precise()
    local pixi = 0
    for i=0, sac.slice_count -1 do
        for j=0, sac.slice_size -1 do
            local p = self.rgb_buf[pixi+1]
            reaper.JS_LICE_PutPixel(self.lice_bitmap, i, sac.slice_size - 1 - j, math.floor(p), 1, "COPY")
            pixi = pixi + 1
        end
    end

    -- reaper.JS_LICE_WritePNG("/Users/ben/Downloads/TOTO.png", lice_bitmap, false)
    LOG.debug("Pixels drawn : " .. pixi .. " in " .. (reaper.time_precise() - t1) .. "\n")
...
function SpectrographWidget:yToNoteNum(my)
    local sac           = self:spectrumContext()
    local uvinfo        = self:xyToUV(0,my)
    local nr            = sac:noteRange()
    return nr.low_note + (1 - uvinfo.v) * (nr.note_count)
end

function SpectrographWidget:noteNumToY(note_num)
    local sac           = self:spectrumContext()
    local nr            = sac:noteRange()
    local alpha         = (note_num - nr.low_note) / nr.note_count
    local u             = 1 - alpha

    -- alpha is on the same scale than vob, vot
    local y = self.y + self.h - (self.vot - u)/(self.vot - self.vob) * self.h
    return y
end
        ...
        if not (mw == 0)  then
            ...
            if UTILS.modifierKeyIsDown() then
                if self:containsPoint(mx, my) then
                    -- Verical ZOOM. Accept verical zoom only if the spectrograph has focus
...
function SpectrographWidget:handleLeftMouse(ctx)
    ...
    -- Fast click without drag : extract profile
...
function SpectrographWidget:handleRightMouse(ctx)
    ...
    -- Fast right click without drag : clear extracted profiles
    if self:containsPoint(mx, my) and (not self.mw:prehemptsMouse()) and not self.lr_mix_widget:containsPoint(mx,my) and ImGui.IsMouseReleased(ctx, ImGui.MouseButton_Right) and (dx == 0 and dy == 0) then
        ...
        if not UTILS.modifierKeyIsDown() then
            ...
            for pi, p in ipairs(self.extracted_profiles) do
                if p.note_num >= note1 and p.note_num <= note2 then
                    ...
                    if not remdst or dst < remdst then
                        torem = pi
                        remdst = dst
                    end
                end
            ...
            if torem then table.remove(self.extracted_profiles, torem) end
...
function SpectrographWidget:drawHorizontalNoteTicks(ctx, draw_list)
    ...
    local sac                   = self:spectrumContext()
    local nr                    = sac:noteRange()
    local intervals             = nr.note_count -- Going from low_note -0.5 to high_note + 0.5, this adds 1 interval
    local vspan                 = self.vot - self.vob
    local ispan                 = (1.0/intervals)  -- Span of an interval in uv coordinates
    local ipixspan              = ispan * self.h / vspan
    local span_is_sufficient    = (ipixspan >= 12)

    for ni = nr.low_note, nr.high_note + 1 do

        local offset    = ni - nr.low_note - 0.5    -- Remove 0.5 for boundary (0 is center, -0.5 is line before and +0.5 is line after)
        local u         = 1 - 1.0 * offset / intervals
        local is_c      = (ni % 12 == 0)

        if (u >= self.vob - ispan) and (u <= self.vot + ispan) then
            local y = self.y + self.h - self.h * (self.vot - u) / vspan
...
function SpectrographWidget:drawProfileLines(ctx)
    ...
    for i, p in pairs(self.extracted_profiles) do
        local y = self:noteNumToY(p.note_num)
        if y > self.y and y < self.y + self.h then
            ImGui.DrawList_AddLine(draw_list, self.x, y, self.x + self.w, y, 0x000000D0, 4)
...
function SpectrographWidget:drawTooltip(ctx)
    ...
    if self.cursor_is_right then
        if self.x + self.w - mx < 2 * w then
            self.cursor_is_right = false
        end
    else
        if mx - self.x < 2 * w then
    ...
    -- Create hysteresis for label position top/bottom of cursor
    -- Depending on border proximity
    if self.cursor_is_top then
        if (my - self.y) < 3 * h then
            self.cursor_is_top = false
        end
    else
        if self.y + self.h - my < 3 * h then
