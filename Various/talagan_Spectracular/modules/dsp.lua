...
local function rmse(samples, dst_rmse, half_window_size)
    local func = getOrCompileFunction("rmse")

    if not #samples == #dst_rmse then
        error("Destination and source should have the same size !")
    end

    ImGui.Function_SetValue(func, "_SIZE",          #samples)
    ImGui.Function_SetValue(func, "_HSIZE",         half_window_size)

    Function_SetValue_ArrayAt(func, "_SAMPLES",     samples,     0)
    ImGui.Function_SetValue(func,   "_RMSE",                     #samples)

    ImGui.Function_Execute(func)

    ImGui.Function_GetValue_Array(func, "_RMSE",    dst_rmse)
end
...
local function analysis_data_to_rgb_array(spectrograms, coeffs, rgb_result, db_min, db_max)
    ...
    for i, spectro in pairs(spectrograms) do
        assert(#spectro == spectro_size, "RGB result is not of the same size of the spectrograms")
    end
