-- @description Convert filtered custom actions to Lua ReaScripts or dump as Lua code.lua
-- @author BuyOne
-- @version 1.2
...
The default search syntax works as boolean OR. Boolean AND search is realized with = operator. Use it between search terms (save for phrases) without spaces when you need several search terms to feature in a custom action name.
...
local lines_t = {}
	...
	-- Weed out duplicates stemming from the same custom action name satisfying several search terms, much simpler than preventing duplicates in the table construction loop above
