...
local function GetInfo()
  ...
  if FX_win then
    ...
    if title:match("FX: Track ") or title:match("FX: Master Track") or title:match("FX: Item ") then
      ...
      return sel_FX, fxid, track, what, trackGUID, take
    else
      return nil
    end
  end
...
------------------------------------------------------------------------------------------------
local function InsertAB(sel_FX, fxid, track, what, take)
  if what == "track" then
    if #sel_FX > 1 then -- enclose selected FXs
      AddTrackAB(track, sel_FX[1])
      AddTrackAB(track, #sel_FX+2)
    ...
    reaper.TrackFX_Show( track, #sel_FX > 1 and #sel_FX+2 or fxid+2, 3 ) -- float AB_LM Receive
    reaper.TrackFX_SetOpen( track, fxid+1, true ) -- keep previously focused FX focused
  elseif what == "item" then
    if #sel_FX > 1 then -- enclose selected FXs
      AddTakeAB(take, sel_FX[1])
      AddTakeAB(take, #sel_FX+2)
    else -- enclose focused FX
      AddTakeAB(take, fxid)
    ...
    reaper.TakeFX_Show( take, #sel_FX > 1 and #sel_FX+2 or fxid+2, 3 ) -- float AB_LM Receive
    reaper.TakeFX_SetOpen( take, fxid+1, true ) -- keep previously focused FX focused
  end
end
...
-- Main function -------------------------------------------------------------------------------
local sel_FX, fxid, track, what, trackGUID, take = GetInfo()
if track and trackGUID then
  ...
  if ok and value == "1" then
    ...
    reaper.Undo_BeginBlock()
    InsertAB(sel_FX, fxid, track, what, take)
