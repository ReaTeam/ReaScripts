...
Icons are identified by their utf8 character combination : "HEX1-HEX2-HEX3-...". Yeah, icons may have be built with a sequence that identifies them, because they can be the result of ligatures. If not, they are simply identified by "HEX". This HEX sequence is called "hexcode" in the specs.
...
- TweMoji doesn't, so I build a new one using this project : https://github.com/Emoji-COLRv0/Emoji-COLRv0
...
The next problem is that these fonts use ligatures because they follow the emoji specification. But ReaImGui doesn't. The idea is to remap virtual glyphs from the fonts to real unicode points. I used free spots in the PUA for this, and wrote a python script that uses fonttools for that purpose. As I'm not at ease with python and the framework, I had to use Claude for this.
...
The TweMoji svg files have naming problems, they use hexcode, but sometimes they do not match the specification. Some characters like ZWJ may be omitted in the hexcode. I do some fallback pirouettes to find the right sequence and re-associate the right icon. It's the "find_real_place_for_icon" function in the index building script.
