...
function copy_take_pitch_env_to_other_items_in_group()
  ...
  elseif
    sel_items > 1 then
    msg("Select one item from a group")
    return
  end
  ...
  local source_env = get_and_show_take_envelope(source_take, "Pitch")
  ...
  for i=1, reaper.CountMediaItems(0) do
    ...
    if item ~= source_item and group > 0 and group == source_group then
      ...
      local env = get_and_show_take_envelope(take, "Pitch")
      local set_ok = reaper.SetEnvelopeStateChunk(env, source_env_chunk, true)
    end
  end
  reaper.Undo_EndBlock("Copy take pitch envelope to other items in the same group", -1)
end


copy_take_pitch_env_to_other_items_in_group()
