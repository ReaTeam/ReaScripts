-- @description Change Take Volume (insert 4 points) for selected items (or selected take vol env) in time selection by set amount (down)
-- @author amagalma
-- @version 1.01
-- @changelog Fix: now working with items with multiple takes too
...
-- @about
--   Lowers the Take Volume for selected items (or selected take vol env if no items are selected) in time selection by a set amount.
--
--   - Inserts 4 envelope points if needed
--   - Shows Take Volume envelope if not already visible
--   - Lowers all existing envelope points in time selection by set amount
