--[[
ReaScript Name: (Un)Tag FX with (in)active envelopes
Author: BuyOne
Version: 1.0
...
About:  As the name suggests the script appends/clears tag 
	...
	BuyOne_(Un)Tag FX with (in)active parameter modulation.lua  
...
local added, removed
	...
	local name_upd
		...
		local env = GetFXEnvelope(obj, fx_idx, i, false) -- create false
			if env and not is_tag then -- add
			added = 1
			name_upd = TAG..' '..name
			break end -- 1 envelope is enough
			if env then is_env = 1 end -- if at least 1 param has env
		end
		if not is_env and is_tag then -- remove // no param has envelope
		removed = 1
		name_upd = name:gsub(Esc(TAG)..'%s','')
		end

		if name_upd then
		chunk = unTag_FX_Name(chunk, fx_GUID, prev_fx_GUID, name, name_upd, video_proc_preset_name) -- chunk string keeps being updated throughout the loop
		end
	end
