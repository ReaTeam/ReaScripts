@description FX Device
@author Bryan Chi
@version 1.0beta
@provides . > BryanChi_FX_Devices
@about Check out the release thread here: https://forum.cockos.com/showthread.php?t=263622
   ...
   --[[  function Link_LT_Param(TrackNumToBeMod, FX_Slt_Indx_ToBeMod, parmlink, PARMLINK_LINKEDPLUGIN,PARMLINK_LINKEDPARMIDX,PARAM_NR, PARMLINK_OFFSET, PARMLINK_SCALE,PARAMOD_BASELINE)
    ...
    ------------------------------------------------------
        ...
        function CallFile(mode, filename, folder)
            local dir_path
            if folder then 
                dir_path = ConcatPath(reaper.GetResourcePath(), 'Scripts', 'FX Devices', folder)
            else 
                dir_path = ConcatPath(reaper.GetResourcePath(), 'Scripts', 'FX Devices')
            end
