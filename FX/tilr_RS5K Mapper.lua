...
-- @author tilr
-- @version 1.0
-- @provides tilr_RS5K Mapper/New file 1.lua > tilr_RS5K Mapper/rtk.lua
-- @screenshot https://raw.githubusercontent.com/tiagolr/rs5kmapper/master/doc/rs5kmapper.gif
-- @about
--   # RS5K Mapper
--
--   https://github.com/tiagolr/rs5kmapper
--
...
function update_widget_drag()
  ...
  if widget_drag.widget == 'pitch' then
    val = widget_drag.start_val - offset_y
    if val < -80 then val = -80
    elseif val > 80 then val = 80
    end
  else
    if val < 0 then val = 0
...
function init()
  ...
  ui_pitch = ui_hbox:add(rtk.Text{'', w=60, cursor=rtk.mouse.cursors.SIZE_NS})
  ui_pitch.onmousedown = function () start_widget_drag('pitch') end
