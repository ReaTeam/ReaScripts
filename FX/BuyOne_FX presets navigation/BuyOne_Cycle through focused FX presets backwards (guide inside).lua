...
-- Replace the text in between the double square brackets
...
local ret, pres_cnt = table.unpack(t)
	...
	r.Undo_BeginBlock()
		...
		Switch_FX_Preset(tr, fx_num, cust_pres_list_t, false) -- forward false
		 _, fx_name = r.TrackFX_GetFXName(tr, fx_num, '') -- for undo caption
		_, pres_name = r.TrackFX_GetPreset(tr, fx_num, '') -- for undo caption
		elseif retval == 2 then
		Switch_FX_Preset(take, fx_num, cust_pres_list_t, false) -- forward false
