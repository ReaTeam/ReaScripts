-- @description Select previous region in region manager window
-- @author Edgemeal
-- @version 1.00
...
function Main()
  ...
  if manager then
    ...
    local sel_region = region_indexes[#region_indexes]

    if sel_region then
      for i = #region_indexes, 1, -1 do -- get current selection & decrement
        local sel = reaper.JS_ListView_GetItemState(lv, region_indexes[i])
        if sel > 1 then
          sel_region = region_indexes[i-1]
          if sel_region == nil then sel_region = region_indexes[#region_indexes] end
          break
        end
      end
