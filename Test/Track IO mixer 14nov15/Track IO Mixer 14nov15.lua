...
--gui.settings.show_receives = true    -- show "receive" knobs/sliders
--gui.settings.show_hw_outs = true     -- show "hardware out" knobs/sliders
--gui.settings.show_sends = true       -- show "send" knobs/sliders
...
gui.settings.io_type_spacing = 40    -- spacing between receives/sends/hw outs
gui.settings.spacing = 5            -- spacing between knobs/sliders
gui.settings.lock_to_track = false    -- false = IO mixer follows track selection
gui.show_receives = true
gui.show_hw_outs = true
gui.show_sends = true
...
local slider_class = require "Slider class 14nov15" -- import "slider class"
...
-- Import menu class
local menu_class = require "Menu class 14nov15"
...
local image_file_table = {
            ...
            {image_path = script_path .. "mcp_volthumb_47x21.png", fr_count = 1}
...
local get_sel_tracks = function () 
                         t = {}
                         for i=1, reaper.CountSelectedTracks(0) do
                           t[#t+1] = reaper.GetSelectedTrack(0, i-1)
                         end
                         return t 
                       end
...
function create_menu()
  ...
  -- Add first submenu item (master track)
  ...
  -- Add submenu               
  ----rc_menu:add_item({label = ">Track list options"})
...
function OnMouseDown(x, y, mouse_cap)
  ...
  --aas = next(gui.last_touched_slider) == nil
  --mouse.capcnt = 0
