--[[
Description: Create time selection at play cursor...
Version: 1.14
Author: Lokasenna
Donation: https://paypal.me/Lokasenna
Changelog:
    Initial release (v1.14 for consistency with edit cursor script)
Links:
...
About:
    ...
    at the play cursor, and can export specific settings as a
    separate action.
Extensions:
--]]
...
local function get_frames(len)
        local rate, drop = reaper.TimeMap_curFrameRate(0)
        return len * 1/rate
end


local function create_time_sel()
    ...
    if position     == "right of" then
        s, e = 0, length
    elseif position == "left of" then
        s, e = -length, 0
    elseif position == "centered on" then
        s, e = -0.5 * length, 0.5 * length
    end
    ...
    -- Create a new time selection
    --startOut retval, endOut reaper.GetSet_LoopTimeRange( isSet, isLoop, startOut, endOut, allowautoseek )
    local ret, _ = reaper.GetSet_LoopTimeRange(true, false, s, e, false)

    local undo_str = string.match(script_filename, "(.+).lua$")
    reaper.Undo_EndBlock( undo_str, 8)

end


if script_filename ~= "Lokasenna_Create time selection at play cursor... .lua" then

    -- Parse vals from filename
    local str = string.match(script_filename, "selection%s(.*)%sthe play cursor")
...
local function set_vals()
    ...
    length = tonumber(GUI.Val("txt_len"))
    _, units = GUI.Val("mnu_units")
    _, position = GUI.Val("mnu_pos")
...
local function btn_save()
    ...
    local name =    "Lokasenna_Create time selection " .. length .. " " .. units ..
                    " long, " .. string.lower(position) .. " the play cursor"
    ...
    arr[1] = "-- This script was generated by Lokasenna_Create time selection at play cursor... .lua\n"
...
GUI.New("lbl_edit", "Label", 1, 196, 47, "the play cursor.")
