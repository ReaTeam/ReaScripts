--[[
  * ReaScript Name: Set time selection to item fade-out
  * Description: Set time selection to item fade-out.
...
-- User settings --
-------------------
local time_sel_start_offset = 1 -- time selection start point from item fade-out start ("pre-roll")
local time_sel_end_offset = 1   -- time selection end point from item fade-out end     ("post-roll")
---------------------------------------------------------------------------------------------------
...
function time_sel_to_fade_in()
  ...
  local fadeout_len = reaper.GetMediaItemInfo_Value(sel_item, "D_FADEOUTLEN")
  local ts_start = item_end-fadeout_len-time_sel_start_offset
  if ts_start-time_sel_start_offset < 0 then ts_start = 0 end
  local ts_end = item_end+time_sel_end_offset
