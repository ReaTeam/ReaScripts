...
-- @author tilr
-- @version 1.0
-- @provides tilr_Keyboard splitter/rtk.lua
...
--   Requires JSFX MIDI Keyvel Filter installed.
...
function create_regions ()
  ...
  for i = 1, reaper.GetNumTracks() do
    ...
    if reaper.IsTrackSelected(track) and not has_keyvel(track) then
      local fxi = reaper.TrackFX_AddByName(track, 'MIDI KeyvelFilter', false, -1000)
      if fxi == -1 then
