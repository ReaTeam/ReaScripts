--[[
ReaScript name: js_Navigate - Select next note in same voice.lua
Version: 0.90
Author: juliansader
...
About:
  # Description
  Similar to REAPER's native action "Navigate: Select next note", but only selects notes within same voice as currently selected note. 
]] 
...
if #tSelNotes == 0 then -- No selected notes?  Simply quit.
    return 

else

    -- Get voice of rightmost selected note
    local lastSelNoteIndex = tSelNotes[#tSelNotes] -- Index inside tNotes of last selected note
    if not tNotes[lastSelNoteIndex].notationIndex then -- If no notation data for this note
        selectedVoice = nil -- Default voice
    else
        -- If default voice, there will be no match, so will also be nil
        selectedVoice = (tEvents[ tNotes[lastSelNoteIndex].notationIndex].msg):match(" [Vv][Oo][Ii][Cc][Ee] (%d+)")
    end
        
    -- Now search ahead for note with matching voice    
    for i = lastSelNoteIndex + 1, #tNotes do
        if not tNotes[i].notationIndex then
            voice = nil
        else
            voice = (tEvents[ tNotes[i].notationIndex].msg):match(" [Vv][Oo][Ii][Cc][Ee] (%d+)")
        end
        if voice == selectedVoice then
    ...
    reaper.Undo_OnStateChange_Item(0, "Select next note in same voice", reaper.GetMediaItemTake_Item(take))
