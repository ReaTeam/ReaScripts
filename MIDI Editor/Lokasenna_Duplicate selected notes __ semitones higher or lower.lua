--[[
Description: Duplicate selected notes ___ semitones higher/lower
Version: 1.0
Author: Lokasenna
...
About: 
	Allows users to type in an interval and duplicate the
	selected notes up or down that many semitones.
Extensions: 
--]]
...
GUI.name = "Type in interval, press Enter"
GUI.x, GUI.y, GUI.w, GUI.h = -48, 0, 250, 30 
...
GUI.elms = {
	...
	txt_interval = GUI.Textbox:new(4, 4, 44, 22, "", 2),
	lbl_semitones = GUI.Label:new(4, 4, "semitones from originals.", 1, 4),
}

GUI.version = nil
GUI.Init()

GUI.elms.txt_interval.focus = true
GUI.font(4)
...
GUI.elms.txt_interval.x = 4 + str_w_a + 4
GUI.elms.lbl_semitones.x = GUI.elms.txt_interval.x + 44 + 4
...
local function Main()
	...
	local cur_focus = GUI.elms.txt_interval.focus
	--GUI.Msg(tostring(last_focus).."  "..tostring(cur_focus))	
	if last_focus == true and cur_focus == false then
		...
		local interval = math.floor(tonumber(GUI.Val("txt_interval"))) or nil
		if not interval then
			GUI.Val("txt_interval", "")
			return 0
		else
