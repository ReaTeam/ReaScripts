...
About:
  ...
  * GRID mode:  In addition to find LFO nodes, the script will also extract CCs that are close to the grid spacing (without quantizing their positions to the grid).
  ...
  5) To finish, left-click, press any keyboard key, or if the shortcut has been held down for more than half a second, simply release the key.
...
local function DEFERLOOP_TrackMouseAndUpdateMIDI()
    ...
    if peekOK and time > prevMouseTime and keys&12 ~= 0 then
        return false
    end
...
function GetAndParseMIDIString()  
    ...
    while nextPos < lastTargetStrPos do
        ...
        if flags&1 == 1 and msg:len() >= 2 and (editAllChannels or msg:byte(1)&0x0F == activeChannel) then -- bit 1: selected
