--[[
ReaScript name: js_Mousewheel - Control js MIDI editing script (if one is running), otherwise zoom vertically.lua
Version: 1.00
Author: juliansader
...
if isnew then
    ...
    elseif sectionID == 0 then 
        if val > 0 then
            reaper.Main_OnCommandEx(40111, -1, 0) -- Main: Zoom in vertical
        elseif val < 0 then
            reaper.Main_OnCommandEx(40112, -1, 0) -- Main: Zoom out vertical
        end 
    ...
    -- (Assuming that this script is only installed in MIDI Editor and Main contexts
        ...
        if val > 0 then
            reaper.MIDIEditor_LastFocused_OnCommand(40111, false) -- Zoom in vertically
        elseif val < 0 then
            reaper.MIDIEditor_LastFocused_OnCommand(40112, false) -- Zoom out vertically
        end
    end
