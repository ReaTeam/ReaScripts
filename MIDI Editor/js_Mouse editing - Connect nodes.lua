...
function AtExit()    
    ...
    if pcallOK == true then        
        ...
        if pcallOK and activeTake and sourceLengthTicks and not (sourceLengthTicks == reaper.BR_GetMidiSourceLenPPQ(activeTake)) then
        ...
        reaper.SetExtState("js_Insert ramps", "Last state", tostring(newEventsAddChannel)..","..mode..","..curve, true)
    end
...
function GetAndParseMIDIString()  
    ...
    for s, i in ipairs(tSortedIndices) do 
        ...
        for a = s+1, #tSortedIndices do 
            if tChannels[tSortedIndices[a]] == tChannels[i] and tPPQs[tSortedIndices[a]] > tPPQs[i] then -- will break after executing "then"
...
function CalculateBezierCurve()
    ...
    for s, i in ipairs(tSortedIndices) do 
        ...
        if tNextInChannel[i] then
            ...
            local PPQ1 = (curSlope == 0) and (curPPQ + (nextPPQ-curPPQ)*0.45) or (curPPQ + (nextPPQ-curPPQ)*0.2)
