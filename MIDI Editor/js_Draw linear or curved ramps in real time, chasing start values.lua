...
local mouseWheel = tonumber(reaper.GetExtState("js_Draw linear", "Last power")) or 1 -- Track mousewheel movement
...
function onexit()
    ...
    reaper.SetExtState("js_Draw linear", "Last power", tostring(mouseWheel), true)
...
function main()
    ...
    if isInline then
        defaultChannel = 0 -- Current versions of REAPER do not provide API access to inline editor default channel.
    else
