...
About:
  # Description
  Arch selected CCs or velocities in lane under mouse towards mouse position, using a linear or power curve.
...
local function loop_trackMouseMovement()
    ...
    if mustDrawCustomCursor then
        ...
        reaper.TrackCtl_SetToolTip(" Λ", mouseXpos+7, mouseYpos+8, true)
    end
    ...
    if mouseWheel >= 1 then
        if mouseNewPPQpos > origPPQleftmost then
            for i = 1, floorIndex do
                ...
                newValue = tableValues[i] + apexHeight*(((tablePPQs[i] - origPPQleftmost)/leftPPQrange)^mouseWheel)
        ...
        if mouseNewPPQpos < origPPQrightmost then
            for i = ceilIndex, #tablePPQs do
                ...
                newValue = tableValues[i] + apexHeight*(((origPPQrightmost - tablePPQs[i])/rightPPQrange)^mouseWheel)
                if newValue > laneMax then newValue = laneMax
                elseif newValue < laneMin then newValue = laneMin
                else newValue = math.floor(newValue+0.5)
                end
        ...
        if mouseNewPPQpos > origPPQleftmost then
            for i = 1, floorIndex do
                ...
                newValue = tableValues[i] + apexHeight - apexHeight*(((mouseNewPPQpos - tablePPQs[i])/leftPPQrange)^inverseWheel)
                if newValue > laneMax then newValue = laneMax
                elseif newValue < laneMin then newValue = laneMin
                else newValue = math.floor(newValue+0.5)
                end
        ...
        if mouseNewPPQpos < origPPQrightmost then
            for i = ceilIndex, #tablePPQs do
                ...
                newValue = tableValues[i] + apexHeight - apexHeight*(((tablePPQs[i] - mouseNewPPQpos)/rightPPQrange)^inverseWheel)
                if newValue > laneMax then newValue = laneMax
                elseif newValue < laneMin then newValue = laneMin
                else newValue = math.floor(newValue+0.5)
                end
