--[[
Description: Duplicate selected notes down an eleventh diatonically
Version: 1.0
Author: Lokasenna
...
About: 
	Duplicates the selected notes at a given scale degree, asking
	the user what to do if it encounters notes outside the scale
Extensions: 
--]]
...
local function dup_notes()
	...
	local val = interval_arr[interval][2]
	...
	if not deg or deg == -1 or deg == 1 then
		...
		reaper.Undo_EndBlock("Duplicate selected notes "..interval_arr[interval][1].." diatonically", -1)
...
local function get_harm_opts(cur_note)
	...
	if not note_dn then note_dn = scale_arr[size] end
	if not note_up then note_up = 12 end	
	
	GUI.Msg("note_dn = "..note_dn.."  note_up = "..note_up)
...
GUI.elms = {
	...
	lbl_num_chrom = GUI.Label:new(		3,	8, 2, "", 0, 3),
...
local function Main()
	...
	-- Has the user made a choice since the last update?
	if cur_num_chroms ~= last_num_chroms then
		...
		GUI.font(GUI.elms.lbl_num_chrom.font)
		local str_w, __ = gfx.measurestr(name_str)
		
		GUI.elms.lbl_num_chrom.x = (GUI.w - str_w) / 2
...
local ret = dup_notes()
if ret ~= -1 and #chrom_notes > 0 then

	GUI.name = "Duplicate selected notes "..interval_arr[interval][1].." diatonically"
	GUI.x, GUI.y, GUI.w, GUI.h = -48, 0, 450, 56
	...
	GUI.elms.btn_harm_up.x = (GUI.w - GUI.elms.btn_harm_up.w) / 2
