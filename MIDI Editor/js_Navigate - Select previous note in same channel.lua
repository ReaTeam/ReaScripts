--[[
ReaScript name: js_Navigate - Select previous note in same channel.lua
Version: 0.90
Author: juliansader
...
About:
  # Description
  Similar to REAPER's native action "Navigate: Select previous note", but only selects notes within same channel as currently selected note. 
]] 
...
while stringPos < MIDIlen do
    ...
    if msg ~= "" then -- Don't need to analyze empty events that simply change PPQ position
        ...
        if eventType == 9 and msg:byte(3) ~= 0 then -- Note-ons (remember that note-ons with velocity=0 re actually note-offs)
...
if #tSelNotes == 0 then -- If no selected notes, simply quit
    ...
    -- Get channel of leftmost selected note
    local firstSelNoteIndex = tSelNotes[1] -- Index inside notes table of last selected note
    local channel = (tEvents[ tNotes[ firstSelNoteIndex].noteOnIndex].msg:byte(1)) & 0x0F
    ...
    reaper.Undo_OnStateChange_Item(0, "Select previous note in same channel", reaper.GetMediaItemTake_Item(take))
