--[[
ReaScript name:  Insert linear ramps between selected CCs or pitches in lane under mouse (without dialog box)
Version: 3.02
Author: juliansader
...
About: 
  ...
  This script is a version of the original "Insert linear or shaped ramps between selected CCs or pitches in lane under mouse" script that
  skips the dialog box and simply inserts linear ramps with default settings.  (NOTE that the default ramp properties and shapes can be customized 
  in the script's USER AREA.)
  ...
  If the "showDialogBox" option is set to "true" in the USER AREA, the script will start with a dialog box in which the user can set:
...
showDialogBox = false -- Should the dialog box be skipped and default values used?  true or false.
...
shape = 1 -- Shape of the ramp.  Either "sine" (or "s"), or a number > 0 for a power curve (1 implies linear).
