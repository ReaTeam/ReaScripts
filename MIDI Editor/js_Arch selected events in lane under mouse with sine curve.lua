...
function arch()  
    ...
    function arch_loop()
        ...
        if (is_new or scriptVal ~= 0 -- mousewheel has been moved
            ...
            --and not (newMousePPQpos <= tableCC[1].PPQ or newMousePPQpos >= tableCC[#tableCC].PPQ) -- don't so anything if move out of event range
            then
            ...
            if wheel >= 1 then -- A power > 1 gives a more musical shape, therefore
                if newMousePPQpos > firstPPQ then      
                    for i = 1, floorIndex do
                        sineValue = 0.5*(1-math.cos(math.pi*(tableCC[i].PPQ - firstPPQ)/leftPPQrange))
                        newValue = tableCC[i].value + apexHeight*(sineValue^wheel)
                ...
                if newMousePPQpos < lastPPQ then                
                    for i = ceilIndex, #tableCC do
                        sineValue = 0.5*(1-math.cos(math.pi*(lastPPQ - tableCC[i].PPQ)/rightPPQrange))
                        newValue = tableCC[i].value + apexHeight*(sineValue^wheel)
                ...
                if newMousePPQpos > firstPPQ then
                    for i = 1, floorIndex do
                        sineValue = 0.5*(1-math.cos(math.pi*(newMousePPQpos - tableCC[i].PPQ)/leftPPQrange))
                        newValue = tableCC[i].value + apexHeight - apexHeight*(sineValue^inverseWheel)
                ...
                if newMousePPQpos < lastPPQ then
                    for i = ceilIndex, #tableCC do
                        sineValue = 0.5*(1-math.cos(math.pi*(tableCC[i].PPQ - newMousePPQpos)/rightPPQrange))
                        newValue = tableCC[i].value + apexHeight - apexHeight*(sineValue^inverseWheel)
...
or mouseLane == 0x201
or (mouseLane == 0x200 or mouseLane == 0x207) 
then
    arch()
else
    return(0)    
