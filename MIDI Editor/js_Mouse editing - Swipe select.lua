...
local _, activeItem, activeTake = nil, nil
 window, segment, details = nil, nil, nil -- given by the SWS function reaper.BR_GetMouseCursorContext()
...
-- Some constants.  In Lua, comaring to local variables is just as fast as comparing to literals
-- mouseOrigCCLane: (CC0-127 = 7-bit CC, 0x100|(0-31) = 14-bit CC, 0x200 = velocity, 0x201 = pitch, 
--    0x202=program, 0x203=channel pressure, 0x204=bank/program select, 
--    0x205=text, 0x206=sysex, 0x207=off velocity)
CC7BIT_MIN    = 0
...
function SetupMIDIEditorInfoFromTakeChunk()
    ...
    --[[
    if not editor then = reaper.Window_GetParent(midiview)  
        while (editor and reaper.MIDIEditor_GetMode(editor) == -1) do 
            editor = reaper.Window_GetParent(editor) 
        end
        if not editor then 
            reaper.MB("Could not determine the MIDI editor under the mouse.", "ERROR", 0) 
            return(false) 
        end
    if not midiview then
        midiview = reaper.JS_Window_FindChild(editor, "midiview", true) or reaper.JS_Window_FindChildByID(editor, 1001)
        if not midiview then 
            reaper.MB("Could not determine the midiview area of the MIDI editor.", "ERROR", 0) 
            return(false) 
        end
    end
    take = reaper.MIDIEditor_GetTake(editor)
        if not (reaper.ValidatePtr2(0, take, "MediaItem_Take*") and reaper.TakeIsMIDI(take)) then 
            reaper.MB("Could not determine a valid active MIDI take for the MIDI editor.", "ERROR", 0) 
            return(false) 
        end
    activeItem = reaper.GetMediaItemTake_Item(take)
        if not reaper.ValidatePtr(item, "MediaItem*") then 
            reaper.MB("Could not determine the item to which the active take belongs.", "ERROR", 0)
            return(false)
        end
    ]]
    ...
    -- If main editor (not inline) get pixel coordinates of editro structure
    if midiview then
        ...
        -- Notes area height is remainder after deducting total CC lane height and height (62 pixels) of Ruler/Marker/Region area at top of midiview
...
function MAIN()          
    ...
    if windowUnderMouse then
        ...
        if parentWindow then
            ...
            elseif parentWindow == reaper.GetMainHwnd() then
                ...
                -- until not (isInline and details == "cc_lane" and mouseOrigCCValue == -1)
                if window == "midi_editor" then --isInline then --reaper.MB("The mouse should be positioned over an inline MIDI editor.", "ERROR", 0) return(false) end
    ...
    if #tTicks == 0 then reaper.MB("No unselected events in target lane(s)."
                                ...
                                .. "\n * This script does not select notes or velocities."
    ...
    local pcallOK, pcallRetval = pcall( 
    function()
