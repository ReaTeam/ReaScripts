...
About:
  # Description
  Similar to REAPER's native action "Navigate: Select previous note", but only selects notes within same voice as currently selected note. 
]] 
...
if #tSelNotes == 0 then -- No selected notes?  Simply quit.
    ...
    -- Get voice of leftmost selected note
    local firstSelNoteIndex = tSelNotes[1] -- Index inside tNotes of last selected note
    if not tNotes[firstSelNoteIndex].notationIndex then -- If no notation data for this note
        selectedVoice = nil -- Default voice
    else
        -- If default voice, there will be no match, so will also be nil
        selectedVoice = (tEvents[ tNotes[firstSelNoteIndex].notationIndex].msg):match(" [Vv][Oo][Ii][Cc][Ee] (%d+)")
    end
        
    -- Now search ahead for note with matching voice    
    for i = firstSelNoteIndex -1, 1, -1 do
    ...
    reaper.Undo_OnStateChange_Item(0, "Select previous note in same voice", reaper.GetMediaItemTake_Item(take))
