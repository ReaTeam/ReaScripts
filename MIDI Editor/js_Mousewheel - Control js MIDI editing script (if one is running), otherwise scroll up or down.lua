--[[
ReaScript name: js_Mousewheel - Control js MIDI editing script (if one is running), otherwise scroll up or down.lua
Version: 1.00
Author: juliansader
...
About:
  ...
  If no js script is running, the script will simply scroll the view up or down, as usual.
...
if isnew then
    ...
    elseif sectionID == 0 then 
        if val > 0 then
            reaper.Main_OnCommandEx(40138, -1, 0) -- Main: Scroll view up
        elseif val < 0 then
            reaper.Main_OnCommandEx(40139, -1, 0) -- Main: Scroll view down
        end 
    ...
    -- (Assuming that this script is only installed in MIDI Editor and Main contexts
        ...
        if val > 0 then
            reaper.MIDIEditor_LastFocused_OnCommand(40138, false) -- Scroll view up
        elseif val < 0 then
            reaper.MIDIEditor_LastFocused_OnCommand(40139, false) -- Scroll view down
        end
    end
