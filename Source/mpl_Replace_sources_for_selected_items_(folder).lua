name_script =  "Replace sources for selected items using user defined folder"
 ...
 --------------------------------------------------------------------------------------  
  ...
  function replace()    
    ...
    for i = 1, #items_t, 1 do
      ...
      for j = 1, #filenames_new_t, 1 do
        filenames_new = filenames_new_t[j]
        filenames_new_clear, filenames_new_ext = filenames_new:match("([^.]+).([^.]+)")
        takename_clear_new = filenames_new_clear       
        src_file_new = input_dir..'\\'..filenames_new
...
function MOUSE_click_under_gui_rect (object_coord_t, offset)   
 ...
 function MOUSE_get()
  ...
  if MOUSE_click_under_gui_rect(but_get2) == true then GET_files_from_dir() end --GET_new_items() end 
 ...
 items_place = {120, but_get[2], 200, #items_t*fontsize}
 
 but_get2 = {10, 150 , 105, 30}
 items_place2 = {120, but_get2[2], 200, #items_new_t*fontsize}
 ...
 function GUI_draw()   
   ...
   GUI_button(but_get2, "Get from folder", fontsize)
   --GUI_info(items_new_t, items_place2)  
   GUI_info(filenames_new_t, items_place2)  
 ...
 function GET_files_from_dir(directory)
   retval, input_dir = reaper.GetUserInputs("", 1, "Directory (backslash in the end): ", "C:\\")  
   if retval ~= nil then filenames_new_t = scandir(input_dir) end
 end  
