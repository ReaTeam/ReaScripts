-- @description Phase align two selected items by moving left item's contents (using CFE)
-- @author amagalma
-- @version 1.00
...
local second_item_take = reaper.GetActiveTake( reaper.GetSelectedMediaItem( 0, 1 ) )
...
local offset = reaper.GetMediaItemTakeInfo_Value( first_item_take, "D_STARTOFFS" )
...
for address in list:gmatch("[^,]+") do
  ...
  if reaper.JS_Window_GetClassName( hwnd ) == "#32770" then
    ...
    reaper.JS_Window_OnCommand( hwnd, 43459 )
    if show then
      reaper.Main_OnCommand(41827, 0) -- Show crossfade editor window
    end
    local loop_cnt = 0
    local function wait()
      if loop_cnt < 3 then
        ...
        local change = offset - reaper.GetMediaItemTakeInfo_Value( first_item_take, "D_STARTOFFS" )
        local source = reaper.GetMediaItemTake_Source( first_item_take )
        if reaper.GetMediaItemInfo_Value( first_item, "B_LOOPSRC" ) == 1 then
        ...
        if change >= 1/samplerate then
          ...
          msg = string.format("Moved left item contents %.1f ms (%i spls) %s",(change*10000+0.5)/10,samples,when)
        end
