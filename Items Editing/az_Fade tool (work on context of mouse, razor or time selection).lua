...
-- @version 2.5.0
-- @changelog
--   - Fix crossfades for looped source (2.4.4 regression)
--   - New option "Fades follow auto-crossfade" (for easy moving one of items to another track)
--   - New option "Treat joint as crossfade for mouse editing" (change crossfade even it has zero length)
--   - New beta-testing mode and option allows editing envelopes in media lane
--   - Regroup options for more clear UI
-- @provides
...
function OptionsDefaults(NamedTable)
  ...
  text = 'Fades follow auto cross-fade'
  table.insert(NamedTable, {text, 'FadesFollowAutoXfade', false })
  
  text = 'Treat joint as crossfade for mouse editing'
  table.insert(NamedTable, {text, 'JointToXfade', false })
  
  text = 'Razor & Time selection'
  table.insert(NamedTable, {text, 'Separator', nil})
  ...
  text = 'Edit single take envelope by razor and mouse instead of double-fades'
  ...
  text = 'Safety & Locking options'
  table.insert(NamedTable, {text, 'Separator', nil })
  
  text = 'Ignore locked items for mouse editing'
  table.insert(NamedTable, {text, 'IgnoreLockingMouse', false })
  
  text = 'Ignore project locking'
  table.insert(NamedTable, {text, 'IgnorePrjLock', false })
   
  text = 'Ignore non-audio items'
  table.insert(NamedTable, {text, 'IgnoreNoAudio', false })
   
  text = 'Beta-test behavior for edit envelopes in media lane'
  table.insert(NamedTable, {text, 'TestMediaLaneEnv', false })
  ...
  text = 'Seek playing back on fade change'
  table.insert(NamedTable, {text, 'SeekOnChange', false })
...
function GetItemsInRange(track, areaStart, areaEnd, areaTop, areaBottom) --returns items, needBatch
    ...
    return items, Litem, Ritem, needBatch, preventFadesOnRests, lockState
end
...
function GetRazorEdits() --returns areaMap, needBatch
    ...
    ---find target envelope (selcted one or single visible envelope)
    local refEnvName, envNameCnt = nil, 0
    local m_x,m_y = reaper.GetMousePosition()
    local m_track, info = reaper.GetThingFromPoint(m_x, m_y)
    local selEnv = reaper.GetSelectedEnvelope(0)
    local m_ret, m_area
    if m_track and info:match('arrange')then
      m_ret, m_area = reaper.GetSetMediaTrackInfo_String(m_track, 'P_RAZOREDITS_EXT', '', false)
    end
    local parTrack
    
    if m_area and m_area ~= '' then
    --[[
      if not MouseCUR then MouseCUR = MouseOverWhat()end
      if MouseCUR == 'razor env' then
        --Doesn't work in media lane:(((
        local retval, takeEnvelope = reaper.BR_GetMouseCursorContext_Envelope() 
        if retval and not takeEnvelope then selEnv = retval end
      end
      ]]
      if selEnv then
        parTrack = reaper.GetEnvelopeInfo_Value( selEnv, 'P_TRACK' )
        EnvRazorPriority = true
      else
        --is there single envelope
        local trEnvCnt = reaper.CountTrackEnvelopes( m_track )
        for i = 0, trEnvCnt -1 do
          local testenv = reaper.GetTrackEnvelope( m_track, i )
          local ret, isSepLane = reaper.GetSetEnvelopeInfo_String( testenv, 'SHOWLANE', '', false )
          local ret, isVis = reaper.GetSetEnvelopeInfo_String( testenv, 'VISIBLE', '', false )
          if tonumber(isSepLane) == 0 and tonumber(isVis) == 1 then
            --only single env is allowed for mouse editing
            if not selEnv then
              selEnv = testenv
              parTrack = m_track
            else selEnv = nil break
            end
          end
        end --for end
    ...
    if m_track and parTrack == m_track and selEnv then
      --find type and ID of the env to match with other tracks
      _, refEnvName = reaper.GetEnvelopeName( selEnv )
      local trEnvCnt = reaper.CountTrackEnvelopes( m_track ) 
      
      for i = 0, trEnvCnt -1 do
        local testenv = reaper.GetTrackEnvelope( m_track, i )
        local testName = reaper.GetEnvelopeName( testenv )
        if refEnvName == testName then envNameCnt = envNameCnt + 1 end
        if selEnv == testenv then break end
      end
    end
    
    if not refEnvName or not Opt.TestMediaLaneEnv then EnvRazorPriority = false end
    ---------------
      ...
      --reaper.ShowConsoleMsg("NEW WAY\n")
        ...
        --msg(area)
          ...
          while i <= #TRareaTable do
            ...
            local isEnvelope = GUID ~= '""' or EnvRazorPriority
            ...
            if not isEnvelope then
              ONLYenvelopes = false
              local TOneedBatch
              items, Litem, Ritem, TOneedBatch, preventFadesOnRests, lockState = GetItemsInRange(track, areaStart, areaEnd, areaTop, areaBottom)
            ...
            if isEnvelope == true or (refEnvName and items == {} ) then
              RAZORenvelopes = true
            end
            --msg(#items)
            if RAZORenvelopes then --msg(refEnvName)
                if refEnvName then
                  local trEnvCnt = reaper.CountTrackEnvelopes( track ) 
                  local nameMatch = 0
                  for e = 0, trEnvCnt -1 do
                    local testenv = reaper.GetTrackEnvelope( track, e )
                    local testName = reaper.GetEnvelopeName( testenv )
                    if refEnvName == testName then nameMatch = nameMatch + 1 end
                    if nameMatch == envNameCnt then
                      envelope = testenv
                      envelopeName = testName
                      break
                    end
                  end
                else
        ...
        if area ~= '' then
          ...
          while j <= #str do
            ...
            if not isEnvelope then 
              ONLYenvelopes = false
              local TOneedBatch
              items, Litem, Ritem, TOneedBatch, preventFadesOnRests, lockState = GetItemsInRange(track, areaStart, areaEnd)
              if needBatch ~= true then needBatch = TOneedBatch end
              if not PreventFadesOnRests then PreventFadesOnRests = preventFadesOnRests end
            end
...
function SetCrossfade(Litem,Ritem,areaData)
  ...
  if LiNewEnd and RiNewStart and LiNewEnd <= RiNewStart then
    ...
    if RunBatch ~= true then
      if GAPanswer == 7 then
        SetFade(Litem, "out", math.abs(LiNewEnd - areaData.areaStart) )
        SetFade(Ritem, "in", math.abs(areaData.areaEnd - RiNewStart) )
      else
  ...
  then
    ...
    if Litem then
      local i_autoFout = reaper.GetMediaItemInfo_Value(Litem, "D_FADEOUTLEN_AUTO")
      
      if MoveItemEdgeRazor or Opt.FadesFollowAutoXfade then
        local outAlen = LiNewEnd - RiNewStart
        if MoveItemEdgeRazor then outAlen = 0 end
        reaper.SetMediaItemInfo_Value( Litem, parOut, LiNewEnd - RiNewStart )
        if not MoveItemEdgeRazor and i_autoFout == 0 and Opt.FadesFollowAutoXfade then
          reaper.SetMediaItemInfo_Value(Litem, "C_FADEOUTSHAPE", DefXfadeShape )
        end
        reaper.SetMediaItemInfo_Value( Litem, parOutA, outAlen  )
      else
        reaper.SetMediaItemInfo_Value( Litem, parOutA, LiNewEnd - RiNewStart )
      end
      
      if not MoveItemEdgeRazor and i_autoFout == 0 then
        reaper.SetMediaItemInfo_Value(Litem, "C_FADEOUTSHAPE", DefXfadeShape )
      end
      
      --- Adjust the opposite fade
    ...
    if Ritem then
      local i_autoFin = reaper.GetMediaItemInfo_Value(Ritem, "D_FADEINLEN_AUTO")
      
      if MoveItemEdgeRazor or Opt.FadesFollowAutoXfade then
        local inAlen = LiNewEnd - RiNewStart
        if MoveItemEdgeRazor then inAlen = 0 end
        reaper.SetMediaItemInfo_Value( Ritem, parIn, LiNewEnd - RiNewStart )
        if not MoveItemEdgeRazor and i_autoFin == 0 and Opt.FadesFollowAutoXfade then
          reaper.SetMediaItemInfo_Value(Ritem, "C_FADEINSHAPE", DefXfadeShape )
        end
        reaper.SetMediaItemInfo_Value( Ritem, parInA, inAlen )
      else
        reaper.SetMediaItemInfo_Value( Ritem, parInA, LiNewEnd - RiNewStart )
      end
      
      if not MoveItemEdgeRazor and i_autoFin == 0 then
        reaper.SetMediaItemInfo_Value(Ritem, "C_FADEINSHAPE", DefXfadeShape )
      end
  ...
  if RiNewStart and LiNewEnd then return math.min(RiNewStart,LiNewEnd)
  elseif RiNewStart then return RiNewStart
  elseif LiNewEnd then return LiNewEnd
  end
end
...
function BatchFades()
  ...
  for i = 1, #RazorEditsBatch do
    ...
    if not areaData.isEnvelope then
      ...
      local lockState = areaData.lockState
      ...
      for i, item in ipairs(items) do
        ...
        if iFin == 0 then iFin = reaper.GetMediaItemInfo_Value( item, "D_FADEINLEN_AUTO" ) end
        if iFout == 0 then iFout = reaper.GetMediaItemInfo_Value( item, "D_FADEOUTLEN_AUTO" ) end
        ...
        if iPos >= areaData.areaStart then
          if PrevEnd and math.abs(iPos - PrevEnd) < 0.0002 then
            ...
            if PrevFout == 0 and iFin == 0
            and ((lockState[i-1] == 0 and lockState[i] == 0) or BOpt.RespectLockingBatch == false ) then
              SetCrossfade(items[i-1],item, crossData)
              AnyEdit = 1
            elseif PrevFout == 0 and (lockState[i-1] == 0 or BOpt.RespectLockingBatch == false) then
              SetFade(items[i-1], "out", BOpt.DefaultFadeOut * typeValueCoeff)
              AnyEdit = 1
            elseif iFin == 0 and (lockState[i] == 0 or BOpt.RespectLockingBatch == false) then
              SetFade(item, "in", BOpt.DefaultFadeIn * typeValueCoeff)
              AnyEdit = 1
            end
            
          elseif not PrevEnd or iPos > PrevEnd then
            if iFin == 0 and (lockState[i] == 0 or BOpt.RespectLockingBatch == false) then
              SetFade(item, "in", BOpt.DefaultFadeIn * typeValueCoeff)
              AnyEdit = 1
            end
            if PrevFout == 0 and (lockState[i-1] == 0 or BOpt.RespectLockingBatch == false) then
              SetFade(items[i-1], "out", BOpt.DefaultFadeOut * typeValueCoeff)
              AnyEdit = 1
            end
          end
          
          if i == #items and iFout == 0 and iEnd <= areaData.areaEnd
          and (lockState[i] == 0 or BOpt.RespectLockingBatch == false) then
    ...
    if needBatch == true then
      ...
      while i > 0 do
        ...
        then
          ...
          if Litem then
            Lilock = reaper.GetMediaItemInfo_Value( Litem, "C_LOCK" )
            Liend = reaper.GetMediaItemInfo_Value( Litem, "D_POSITION" )
                  + reaper.GetMediaItemInfo_Value( Litem, "D_LENGTH" )
          end
          
          if Ritem then
            Rilock = reaper.GetMediaItemInfo_Value( Ritem, "C_LOCK" )
            Ristart = reaper.GetMediaItemInfo_Value( Ritem, "D_POSITION" )
          end
          
          if IgnoreLockedItems ~= true then
            if Lilock == 1 then Litem = nil end
            if Rilock == 1 then Ritem = nil end
          end
          ...
          if #items == 2 then
            ...
            if Litem and Ritem then
              --create crossfade
              if edgesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false then return end
              table.insert(fadeStartT, areaData.areaStart)
              SetCrossfade(Litem,Ritem,areaData)
            end
            
            if not Ritem then
              if fadesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false then return end
              MoveItemEdgeRazor = true
              table.insert(fadeStartT, math.max(Liend, Ristart))
              SetCrossfade(Litem,nil,{areaEnd = math.min(math.max(Liend, Ristart), areaData.areaEnd ),
                                      areaStart = areaData.areaStart})
            end
            
            if not Litem then
              if fadesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false then return end
              MoveItemEdgeRazor = true
              table.insert(fadeStartT, math.min(Liend, Ristart))
              SetCrossfade(nil, Ritem,{areaStart = math.max( math.min(Liend, Ristart), areaData.areaStart ),
                                      areaEnd = areaData.areaEnd})
            end
          end
          -------End of the 1st case-----------
          ...
          if #items == 1 then
            ...
            if Ritem and Litem == nil then  --fade IN
              ...
              if Opt.MoveItemEdgeRazor == true then
                table.insert(fadeStartT, areaData.areaStart)
                SetCrossfade(nil, item, areaData)
              else
            ...
            elseif Litem and Ritem == nil then -- fade OUT
              ...
              if Opt.MoveItemEdgeRazor == true then
                table.insert(fadeStartT, areaData.areaStart)
                SetCrossfade(item, nil, areaData)
              else
  ...
  for try = 1, 2 do
    if try == 1 or ( #SI == 0 and Opt.AllowNoTimeRazorBatch) then
      ...
      for i=0, reaper.CountSelectedMediaItems(0) -1 do
        local TData = {Tname, Titems={}, lockState={} }
        ...
        if item then
          if iTrack ~= oldTrack then
            ...
            table.insert(TData.lockState, iLock)
            table.insert(SI, TData)
          else
            if not FieldMatch(SI[#SI].Titems, item, true ) then
              table.insert(SI[#SI].lockState, iLock)
            end
          end
...
if item_under_mouse then
  ...
  local ilock = reaper.GetMediaItemInfo_Value( item_under_mouse, "C_LOCK" )
  if ilock == 1 and Opt.IgnoreLockingMouse == true then
    IgnoreLockedItems = true
  end
...
function FindXfadedNeigbourItem(item, i_pos, i_end, step) --step decreasing or increasing item number
  ...
  while nextItem do
    ...
    if nextItem then
      ...
      if Opt.JointToXfade then
        if ((round(ni_end - i_pos, 3) >= 0 and ni_end < i_end) or (ni_pos > i_pos and round(i_end - ni_pos, 3) >= 0 ))
        and ( ni_Y < i_Y + i_H and ni_Y + ni_H > i_Y )
        then
         return nextItem, ni_pos, ni_end
        end
      else
...
function SortSelItems(Items, SelItems, ref_item, ref_leftItem, ref_rightItem, reverseFlag)
  ...
  for i, item in ipairs(SelItems) do
    ...
    if i_autoFin ~= 0 or Opt.JointToXfade then
    ...
    if i_autoFout ~= 0 or Opt.JointToXfade then
    ...
    if leftIlock == 1 and Opt.IgnoreLockingMouse ~= true then leftItem = nil end
    if rightIlock == 1 and Opt.IgnoreLockingMouse ~= true then rightItem = nil end
    if itemLock == 1 and Opt.IgnoreLockingMouse ~= true then item = nil end
...
function AddTrMediaEditingGroup(Items, timeT, edge)
  ...
  -- 
  if extract_version(reapVers) >= '7.43' then
    for o,v in ipairs(offsets) do
  ...
  for t, time in ipairs(timeT) do
    ...
    for i, tr in ipairs(GrTracks) do
      ...
      for k = idx, itemCount do
        ...
        if ipos > time + 0.0002 and edge ~= nil then break end
...
function FadeToMouse(item, itemHalf) --returns table of fades start position
  local ilock = reaper.GetMediaItemInfo_Value( item, "C_LOCK" )
  if fulliLock == 1 then return end
  if ilock == 1 and IgnoreLockedItems ~= true then return end
  ...
  if Grouping == 1 then
    ...
    local isItemSel = reaper.IsMediaItemSelected(item)
    local isNghSel
    if neighItem then isNghSel = reaper.IsMediaItemSelected(neighItem) end
    
    if isItemSel == false then
      if isNghSel ~= true then
        reaper.SelectAllMediaItems(0, false)
        SelItems = {item}
      else 
        local step
        if edge == 'l' then step = -1  else step = 1 end
        for i, nghItem in ipairs(Sitems) do
          local ngPos = reaper.GetMediaItemInfo_Value(nghItem, "D_POSITION")
          local ngEnd = ngPos + reaper.GetMediaItemInfo_Value(nghItem, "D_LENGTH")
          local origItem = FindXfadedNeigbourItem(nghItem, ngPos, ngEnd, -step) 
          if origItem and reaper.IsMediaItemSelected(origItem) == false then
            FieldMatch(SelItems, origItem, true)
          else FieldMatch(SelItems, nghItem, false)
          end
        end
      end
      
    else SelItems = SaveSelItems()
    end
    
    SelItems = AddGroupedItems(SelItems, false, time, edge) --To be consistent with split script and native reaper behavior
  ...
  if itemHalf == 'header' then --MOVE CLOSEST XFADE TO MOUSE 
    if edgesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false then
      return
    end
  ...
  if f_type == 'in' and leftItem then --change crossfade
    ...
    if ( Opt.IgnoreLockingMouse ~= true and leftIlock == 1 )
    or (edgesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false) then
      return
    end
  ...
  elseif f_type == 'out' and rightItem then --change cdrossfade
    if ( Opt.IgnoreLockingMouse ~= true and rightIlock == 1 )
    or (edgesLock == 1 and globalLock == 1 and Opt.IgnorePrjLock == false) then
      return
    end
...
function AddGroupedItems(itemsTable, retWithoutInputs, matchTime, matchEdge) --table, boolean
  local grItems = {}
  local grIDs, neighGrIDs = {}, {}
  local allCnt = reaper.CountMediaItems(0)
  local inpCnt = #itemsTable
  local step
  if matchEdge == 'l' then step = -1 else step = 1 end
  
  for i = 1, inpCnt do
    local item = itemsTable[i]
    local groupID = reaper.GetMediaItemInfo_Value(item, 'I_GROUPID') 
    if groupID ~= 0 then FieldMatch(grIDs, groupID, true) end
    
    local iPos = reaper.GetMediaItemInfo_Value(item, 'D_POSITION')
    local iEnd = iPos + reaper.GetMediaItemInfo_Value(item, 'D_LENGTH')
    local neighbourItem = FindXfadedNeigbourItem(item, iPos, iEnd, step)
    if neighbourItem then
      local neighGr = reaper.GetMediaItemInfo_Value(neighbourItem, 'I_GROUPID')
      if neighGr ~= 0 then FieldMatch(neighGrIDs, neighGr, true) end
    end
  end
  
  if #grIDs > 0 or #neighGrIDs > 0 then
    for i = 0, allCnt-1 do
      ...
      if groupID ~= 0 then
        ...
        if FieldMatch(grIDs, groupID) == true then
          local itemTime = iPos
          if matchEdge == 'r' then itemTime = iEnd end
        ...
        elseif FieldMatch(neighGrIDs, groupID) == true then 
          local crossedWithGrNeigh, ngPos, ngEnd = FindXfadedNeigbourItem(item, iPos, iEnd, -step)
          local itemTime = ngPos
          if matchEdge == 'r' then itemTime = ngEnd end
          if matchTime and math.abs(matchTime - itemTime) < 0.0002 then
            table.insert(grItems, crossedWithGrNeigh)
          elseif not matchTime then table.insert(grItems, crossedWithGrNeigh) --???
          end
        end
      end
    end
...
function MoveEditCursor(timeTable)
  if #timeTable > 0 then
    ...
    if Opt.moveEditCursor == true then
      reaper.SetEditCurPos2(0, fadeStartEdge - Opt.curOffset, false, Opt.SeekOnChange)
    else
      if Opt.SeekOnChange then
        reaper.SetEditCurPos2(0, fadeStartEdge - Opt.curOffset, false, true)
      end
...
function PointToMouse(env) --return time table with single point value
  ...
  if envItem ~= 0 then
    if IgnoreLockedItems ~= true and takeEnvLock == 1 then return end
    ...
    if envItem == item_mouse and itemHalf ~= 'header' then
     ...
     if IgnoreLockedItems ~= true and envLock == 1 then return end
     local x,y = reaper.GetMousePosition()
     
     local OScoeff = 1
     if reaper.GetOS():match("^Win") == nil then OScoeff = -1 end
     
     local envYpos = reaper.GetEnvelopeInfo_Value( env, 'I_TCPY' ) --msg(envYpos)
     local envTrack = reaper.GetEnvelopeInfo_Value( env, 'P_TRACK' )
     local envTrackH = reaper.GetMediaTrackInfo_Value( envTrack, 'I_TCPH' ) --msg(envTrackH)
     local envHbig = reaper.GetEnvelopeInfo_Value( env, 'I_TCPH' ) --msg(envHbig)
     local envH = reaper.GetEnvelopeInfo_Value( env, 'I_TCPH_USED' ) --msg(envH)
     local envPad = envHbig - envH
     local mouseEnv
     local testEnv
     
     local track, infostr
     local testTrack, testInfostr
     
     if envYpos < envTrackH then --msg('env in media lane')
       track, infostr = reaper.GetThingFromPoint(x,y)
       local test_y = math.floor( y + (envTrackH/2 - envPad)*OScoeff )
       testTrack, testInfostr = reaper.GetThingFromPoint(x, test_y )
     ...
     --msg(tostring(infostr)..' '.. tostring(testInfostr) )
     local mouse_time = reaper.BR_GetMouseCursorContext_Position()
     
     if (env == mouseEnv and env == testEnv)
     or (envTrack == track and envTrack == testTrack and infostr == testInfostr) then --top half
     --msg('top')
...
local ret, var
ret, var = reaper.get_config_var_string( 'defxfadeshape' )
if ret then DefXfadeShape = tonumber(var) end
...
if RazorEditSelectionExists() == true then
  ...
  if env then --msg('env')
    local ret, isVis = reaper.GetSetEnvelopeInfo_String( env, 'VISIBLE', '', false )
    if isVis == '1' then
      Attempt = true
      reaper.Undo_BeginBlock2( 0 )
      reaper.PreventUIRefresh( 1 )
  ...
  if UndoString == nil then
    --msg(not (reaper.IsMediaItemSelected(item_mouse) and pixDist == 'close' ))
    if item_mouse
    and not ( reaper.IsMediaItemSelected(item_mouse) and pixDist == 'close' ) then
      ...
      if info:match('envelope') then
        ...
        reaper.PreventUIRefresh( 1 )
        sTime = PointToMouse(env)
      elseif info:match('arrange') and Opt.TestMediaLaneEnv then
        local env
        local trEnvCnt = reaper.CountTrackEnvelopes( track )
        for i = 0, trEnvCnt -1 do
          local testenv = reaper.GetTrackEnvelope( track, i )
          local ret, isSepLane = reaper.GetSetEnvelopeInfo_String( testenv, 'SHOWLANE', '', false )
          local ret, isVis = reaper.GetSetEnvelopeInfo_String( testenv, 'VISIBLE', '', false )
          --msg(isSepLane ..' '.. isVis)
          if tonumber(isSepLane) == 0 and tonumber(isVis) == 1 then
            --only single env is allowed for mouse editing
            if not env then env = testenv
            else env = nil break
            end
          end
        end
  ...
  if reaper.CountSelectedMediaItems(0) > 0 and UndoString == nil and not Attempt
  and (not item_mouse or reaper.IsMediaItemSelected(item_mouse) )then
    local areaMap, needBatch
    
    reaper.Undo_BeginBlock2( 0 )
    reaper.PreventUIRefresh( 1 )
    if Grouping == 1 then
      reaper.Main_OnCommandEx(40034,0,0) --Item grouping: Select all items in groups
      --twice for workaround the reaper's bug
      reaper.Main_OnCommandEx(40034,0,0) --Item grouping: Select all items in groups
    end
    
    if pixDist == 'close' then
      areaMap, needBatch = GetTSandItems(start_TS, end_TS)
    else
      areaMap, needBatch = GetTSandItems(0, 0)
    end
    
    sTime = FadeRazorEdits(areaMap, needBatch)
    if not RunBatch then RestoreLockedItems() end
