...
-- @author Rek's Effeks
-- @version 1.0
-- @changelog Created the scripts. Thank you to ReaTrak for providing a baseline.
-- @metapackage
-- @provides
--   [linux main] rekseffeks_SpleeterLinux/Spleeter2.lua
--   [linux main] rekseffeks_SpleeterLinux/Spleeter4.lua
--   [linux main] rekseffeks_SpleeterLinux/Spleeter5.lua
-- @link
...
if not end_script then
	...
	local cmd = ('spleeter separate -p spleeter:2stems-16kHz -o \"' .. getPath(filepath) .. '\" \"' .. filename .. '\"')

	if reaper.MB( "       Splitting selected audio into 2 stems.\n\n                 Vocals / Accompaniment\n\nProgram will hang until task is done. Continue?", "Spleeter confirmation", 1 ) == 1 then
		reaper.ExecProcess(cmd, 0)
		reaper.InsertMedia(filename_no_ext .. "/vocals.wav", 1)
		reaper.InsertMedia(filename_no_ext .. "/accompaniment.wav", 1)
		reaper.MB( "Done.", "Spleeter", 0 )
	end
end

reaper.Undo_EndBlock("Spleeter (2 stems)",0)
