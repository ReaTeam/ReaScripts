-- @description Smart split MIDI item (trim left note parts at cursor)
-- @author solger
-- @version 1.0
-- @changelog First version
-- @about
--   This script splits a MIDI item and trims the left note parts at the cursor position. The threshold of the left note part length which should be trimmed can be adjusted in the script.
--
...
if selectedItem == nil then
  ...
  for i = noteCount-1, 0, -1 do
    local _, selected, _, startPPQ, _, _, _, _ = r.MIDI_GetNote(currentTake, i)
    if selected then
      local cursorPPQ = math.floor(r.MIDI_GetPPQPosFromProjTime(currentTake, r.GetCursorPosition()) + 0.5)
        if trimThreshold > 0 and r.MIDI_GetProjQNFromPPQPos(currentTake, cursorPPQ - startPPQ) > trimThreshold then
...
r.PreventUIRefresh(-1)
r.Undo_EndBlock('Smart split MIDI item (trim left note parts at cursor)', -1)
