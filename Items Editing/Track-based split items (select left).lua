-- @description Track-based split items (select left)
-- @author Triode
-- @version 0.64
...
-- @about
--   Map this to a (modified) media item click in mouse modifiers to allow splitting of items based on the folder or track group they are contained in.  Also includes individual item splitting and splitting of items across all tracks.
--
...
--
--   This script is for item splitting (select left).  Download the other track based scripts to get item selection, range selection (shift-click), toggle selection, item split (select right) using the same modes.
--
...
boundary = folder_boundary + group_boundary
...
--------------------Select All mode
 ...
 -- reaper.Main_OnCommand(reaper.NamedCommandLookup("_WOL_SELTREXCFOLDPAR"), 0)  -- select all tracks except folder parents (undo related)
...
--split items
  ...
  reaper.Main_OnCommand(40758,0) -- split items at edit cursor (select left)
  
  function Items_Right_of_Cursor(item)  -- --- thankyou Edgemeal
    ...
    if cursor < ending - (fadein) then return false end
    return true
  end
  ...
  for i = item_count-1, 0, -1 do
    local item = reaper.GetSelectedMediaItem(0, i)
    reaper.SetMediaItemSelected(item, Items_Right_of_Cursor(item))  -- unselect items ending RIGHT of cursor 
  end
...
--reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWS_AWSELGROUPIFGROUP"), 0)  --- select all items in groups if grouping is enabled
end

    --reaper.ShowMessageBox(groupID, "Item Groups", 0) -- just to test
reaper.Undo_EndBlock('Track-based Split Items (Select Left)', -1)
