--[[
  @author bfut
  @version 1.0
  @description Unselect items touching time selection
  @about
    HOW TO USE:
      1) Run the script.
...
for i = COUNT_SEL_ITEMS - 1, 0, -1 do
  ...
  if pos + reaper.GetMediaItemInfo_Value(item, "D_LENGTH") - ORIGINAL_TIMESEL_START >= 0 and
      ORIGINAL_TIMESEL_END - pos >= 0 then
    reaper.SetMediaItemSelected(item, false)
  end
end
reaper.PreventUIRefresh(-1)
reaper.UpdateArrange()
reaper.Undo_EndBlock2(0, "bfut_Unselect items touching time selection", -1)
