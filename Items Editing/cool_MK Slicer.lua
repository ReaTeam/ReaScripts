...
-- @author cool
-- @version 1.2
-- @changelog 1.2 - first tested public version.
-- @provides [main] cool_MK Slicer/cool_MK Slicer (Non-Destructive).lua
...
--------------------------Set Item's True Starting Position----------------------------
function start_position()

local function start_pos(item)
  local take = r.GetActiveTake(item)
  local retval, offs, leng, revs = r.PCM_Source_GetSectionInfo( r.GetMediaItemTake_Source( take ) )
  local retval, section, PItemStart, PItemLen, fade, bobby = r.BR_GetMediaSourceProperties( take )
  r.BR_SetMediaSourceProperties( take, false, PItemStart, PItemLen, 0, false )
...
local XButton,ZButton, Button, Slider, Rng_Slider, Knob, CheckBox, Frame, ErrMsg, Txt = {},{},{},{},{},{},{},{},{},{}
  ...
  extended(Txt,     Element)
    ...
    local H_Slider, V_Slider, T_Slider, HP_Slider, LP_Slider, G_Slider, S_Slider, Rtg_Slider, Rdc_Slider, O_Slider = {},{},{},{},{},{},{},{},{},{}
...
local Midi_Sampler = CheckBox:new(610,410,68,18, 0.3,0.4,0.7,0.7, "","Arial",16,  1,
                              {"Sampler","Trigger"} )
...
function HP_Freq:draw_val()
  ...
  self.form_val = floor(math.exp(sx*math.log(1.059))*8.17742) -- form val
  -------------
  local x,y,w,h  = self.x,self.y,self.w,self.h
...
-- Filter Gain -----------------------------------
local Fltr_Gain = G_Slider:new(20,450,160,18,  0.3,0.4,0.7,0.7, "View Gain","Arial",16, out_gain )
...
local Slider_TB = { HP_Freq,LP_Freq,Fltr_Gain, 
                   Gate_Thresh,Gate_Sensitivity,Gate_Retrig,Gate_ReducePoints,Offset_Sld}
...
local Slider_TB_Trigger = { HP_Freq,LP_Freq,Fltr_Gain, 
                   ...
                   Gate_VeloScale, VeloMode,OutNote, Velocity,Offset_Sld}
...
local Get_Sel_ErrMsg = ErrMsg:new(560,452,260,25, 1, 1, 1, 1, "Only single track items, please",    "Arial", 22)
...
local function Msg()
   local char = gfx.getchar()
     if char == 27 or char == -1 or (os.time() - time) > timer then return end
local Get_Sel_ErrMsg = ErrMsg:new(560,452,260,25, 1, 1, 1, 1, "Only Wave items, please",    "Arial", 22)
local ErrMsg_TB = {Get_Sel_ErrMsg}
     for key,btn    in pairs(ErrMsg_TB)   do btn:draw()    
   gfx.update()
  r.defer(Msg)
...
if selected_tracks_count > 1 and count_itms == selected_tracks_count then
  ...
  r.Main_OnCommand(41844, 0) -- Remove Markers
   r.Undo_EndBlock("Reset (multi1)", -1)    
...
select_first_item()
               r.Main_OnCommand(r.NamedCommandLookup('_SWS_TOGITEMSEL'), 0) -- Inverse Item Selection
               r.Main_OnCommand(40312, 0) -- Remove All Selected
               r.Main_OnCommand(40718, 0) -- Select all items on selected tracks in current time selection
               r.Main_OnCommand(42228, 0) -- Restore Source Duration
               r.Main_OnCommand(r.NamedCommandLookup('_SWS_AWTRIMCROP'), 0) -- TRIM/CROP
               r.Main_OnCommand(41385, 0) -- Fit items to time selection
               r.Main_OnCommand(40634, 0) -- Remove Loop Point Selection
  ...
  r.Undo_EndBlock("Reset (multi2)", -1)    
end
...
local function Msg()
   local char = gfx.getchar()
     if char == 27 or char == -1 or (os.time() - time) > timer then return end
local Get_Sel_ErrMsg = ErrMsg:new(560,452,260,25, 1, 1, 1, 1, "Something went wrong. Use Undo (Ctrl+Z)",    "Arial", 22)
local ErrMsg_TB = {Get_Sel_ErrMsg}
     for key,btn    in pairs(ErrMsg_TB)   do btn:draw()    
   gfx.update()
  r.defer(Msg)
...
if SliceQ_Status ~= 0 then
 ...
 count_itms =  r.CountSelectedMediaItems(0)
   ...
   r.Main_OnCommand(r.NamedCommandLookup("_SWS_QUANTITESTART2"),0) -- quantize

cleanup_slices()

  r.Main_OnCommand(r.NamedCommandLookup("_SWS_AWFILLGAPSADVLASTSETTINGS"),0) -- fill gaps (advanced) --make crossfades

       r.GetSetProjectGrid(proj, true, save_project_grid, save_swing, save_swing_amt) -- restore saved grid settings

   restorepos() -- restore item position
...
r.PreventUIRefresh(1)
   -------------------------------------------
    ...
    if save_project_grid > 0.5 then
       ...
       r.Main_OnCommand(41846, 0)  ---Snap Markers to Grid
...
function Wave:Create_MIDI()
  ...
  -------------------------------------------
    ...
    for i=1, points_cnt, 2 do
        ...
        r.MIDI_InsertNote(take, sel, mute, startppqpos, endppqpos, chan, pitch, vel, true)
    end
    ----------------------------
...
function Init()
    ...
    local Wnd_Title = "MK Slicer v1.2"
    local Wnd_Dock, Wnd_X,Wnd_Y = 0,400,320 
