...
-- @author 80icio
-- @version 0.21 (Beta)
-- @screenshot Forum Link https://forum.cockos.com/showthread.php?p=2764162#post2764162
-- @about
--   Perfect Timing! is a straightforward audio quantizer with 2 modes, SPLIT and WARP
--   It features:
--
--   - Fast transient analysis and 2 visualization options.
--   - Timesaving Grid Based transient detection.
--   - Multitrack and Multichannel analysis and group editing.
--   - Hipass and Lowpass filters and transient attack for enhanced transient searching.
--   - Fast quantizing + crossfading.
...
if not dofile(r.GetResourcePath() .. '/Scripts/ReaTeam Extensions/API/imgui.lua') then ------------IMGUI check-----------------------
  ...
  if REAIMGUI_VERSION < 83  then ------------ IMGUI version check-----
    r.ShowMessageBox( "Please update IMGUI extension via REAPACK.\nThanks!", "Ooops!", 0 )
    r.ReaPack_BrowsePackages('ReaImGui: ReaScript binding for Dear ImGui')
    return false
  end
